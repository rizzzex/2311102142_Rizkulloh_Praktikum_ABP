<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2 text-sm">
            ...
            <span class="text-white uppercase tracking-widest font-black text-[10px]">Modifikasi Data</span>
            <span class="text-gray-500">/</span>
            <span class="text-crimson uppercase tracking-widest font-black text-[10px]">#{{ $product->id }}</span>
        </div>
    </x-slot>
    ...
    <div class="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div class="glass-card rounded-[2.5rem] overflow-hidden border-t-4 border-crimson">
            <div class="px-10 py-8 border-b border-white/5">
                <h3 class="text-2xl font-black text-white uppercase tracking-tighter">Modifikasi Item</h3>
                <p class="mt-1 text-sm text-gray-500">Anda sedang memperbarui parameter untuk: <span class="text-white font-bold">{{ $product->name }}</span></p>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-3">
                        ...
                        <select name="category" id="category" 
                            ...
                            <option value="Electronics" {{ old('category', $product->category) == 'Electronics' ? 'selected' : '' }} class="bg-crimson-dark-950">Elektronik</option>
                            <option value="Clothing" {{ old('category', $product->category) == 'Clothing' ? 'selected' : '' }} class="bg-crimson-dark-950">Pakaian</option>
                            <option value="Food" {{ old('category', $product->category) == 'Food' ? 'selected' : '' }} class="bg-crimson-dark-950">Makanan</option>
                            <option value="Home" {{ old('category', $product->category) == 'Home' ? 'selected' : '' }} class="bg-crimson-dark-950">Kebutuhan Rumah</option>
                            <option value="Other" {{ old('category', $product->category) == 'Other' ? 'selected' : '' }} class="bg-crimson-dark-950">Lainnya</option>
                        </select>
                    ...
                    <!-- Stok -->
                    <div class="space-y-3">
                        <label for="stock" class="block text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">Stok Tersedia <span class="text-crimson">*</span></label>
